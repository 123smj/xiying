<%@ page language="java" import="com.trade.bean.response.ThirdPartyPayResponse,com.trade.enums.*" pageEncoding="UTF-8"%>
<%
...
<html>
	...
	<body>
		...
		<%if(thirdPartyPayResponse == null) {%>
			<h2 class="message" i18n-data="i18n">该支付服务未开通，请联系业务员开通[code:0009]</h2>
		<%} else if(!ResponseEnum.SUCCESS.getCode().equals(thirdPartyPayResponse.getResultCode())){%>
	  		<h2 class="message" i18n-data="i18n"><%=thirdPartyPayResponse.getMessage() %>[code:<%=thirdPartyPayResponse.getResultCode() %>]</h2>
		<%}%>
	</body>
</html>