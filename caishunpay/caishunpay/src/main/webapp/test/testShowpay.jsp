...
<html>
	<head>
	    <title><%=TradeSource.get(tradeSource)==null? "": TradeSource.get(tradeSource).getMemo()%></title>
...
<%--		<link rel='stylesheet' href="<%= request.getContextPath()%>/stylesheets/iPhone4.css" media='only screen and (-webkit-min-device-pixel-ratio: 1.5)' >--%>
	...
	<body>
		...
		<div align="center" >交易金额：<strong><%= StringUtil.changeF2Y(Integer.valueOf(orderAmount))%></strong></div>
		<h2 class="message" ><font color="red">打开<%=TradeSource.get(tradeSource)==null? "": TradeSource.get(tradeSource).getMemo()%>，扫一扫支付</font></h2>
	</body>
	<script type="text/javascript">
			var url = "<%= thirdPartyPayResponse.getPay_url()%>";
