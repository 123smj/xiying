...
<body  onload="IFrameResize()" class=" theme-blue" >
					...
					<table class="table" width="1500px" height="auto">
						<thead style="height: 20px">
							<tr style="height: 20px">
								...
								<th>
									<span style="width: 100px; display: block;">业务方式</span>
								</th>
								<th>
									<span style="width: 90px; display: block;">商户号</span>
								</th>
								<th>
									<span style="width: 90px; display: block;">商户名称</span>
								</th>
								<th>
									<span style="width: 90px; display: block;">账目序列号</span>
								</th>
								<th>
									<span style="width: 100px; display: block;">交易金额</span>
								</th>
								<th>
									<span style="width: 100px; display: block;">设置额度</span>
								</th>
								<th>
									<span style="width: 120px; display: block;">状态</span>
								</th>
								<th>
	...
	function accountFileQuery(num){
		...
		$.ajax({
            cache: true,
            type: "POST",
            url:"manage/accountFileDetail?r=" + new Date().getTime(),
            ...
            success: function(data) {
                ...
                for(var i=0; i<data.data.length; i++){
                	...
                	item.offerSeq + '</td><td>' +
                	'<span class="span_auto_width"><a class="text_green" href="<%= request.getContextPath()%>/manage/ajaxDownLoad?path=<%= SysParamUtil.getParam("mcht_account_file")%>' + item.opr_time.substring(0,8) + "/" + item.offerDoc + '">' + item.offerDoc + '</a></span></td><td>' +
                	'<span class="span_auto_width">' + item.trade_type + '</span></td><td>' +
                	'<span class="span_auto_width">' + item.mcht_no + '</span></td><td>' +
                	'<span class="span_auto_width">' + item.mcht_name + '</span></td><td>' + 
                	'<span class="span_auto_width">' + item.dfsn + '</span></td><td>' + 
                	'<span class="span_auto_width">' + item.trade_amount + '</span></td><td>' + 
                	'<span class="span_auto_width">' + item.mcht_income + '</span></td><td>' + 
                	'<span class="span_auto_width">' + accountStatus(item.status) + '</span></td><td>' +
                	'<span class="span_auto_width" >' + item.opr_time + '</span></td>';
