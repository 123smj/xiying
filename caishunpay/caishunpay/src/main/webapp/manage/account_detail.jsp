...
<div>
		...
		<div class="search-well">
                <form class="form-inline" id="account_form" name="account_form" style="margin-top:0px;margin-left: 10px" action="manage/accountDetail" target="_self">
                	<table >
                		<tr>
                    	...
                    	<td><span><label class="control-label span_auto_width" for="lunch">代付状态：</label></span></td><td>
                    	...
                    	<td><label>商户代付单号：</label></td><td>
                    		<input name="dfSn" id="tradeSn" class="input-xlarge form-control" placeholder="商户代付单号" type="text" ></td>
                    	<td>
            				<label>平台代付单号：</label></td><td>
                    		<input name="dfTransactionId" id="dfTransactionId" class="input-xlarge form-control" placeholder="平台代付单号"  type="text">	
          				</td>
          				<td>
            				<label>通道返回码：</label></td><td>
                    		<input name="dfStatus" id="dfStatus" class="input-xlarge form-control" placeholder="通道返回码"  type="text">	
          				</td>
                    	</tr>
                    	<tr>
                    	<td>
            				<label>收款人：</label></td><td>
                    		<input name="receiptName" id="receiptName" class="input-xlarge form-control" placeholder="收款人"  type="text">	
          				</td>
					...
					<table class="table" width="1500px" height="auto">
						<thead style="height: 20px">
							<tr style="height: 20px">
								...
								<th>
									<span style="width: 120px; display: block;">商户名称</span>
								</th>
								<th>
								...
								<th>
									<span style="width: 120px;display: block;">通道返回描述</span>
								</th>
								<th>
		...
		$.ajax({
            ...
            success: function(data) {
                ...
                for(var i=0; i<data.data.length; i++){
                	...
                	'<span class="span_auto_width">' + item.dfSn + '</span></td><td>' +
                	item.accountOrderNo + '</td><td>' +
                	'<span style="display: inline-block; overflow: hidden;white-space:nowrap;" >' + item.receiptBankNm + '</span></td><td>' + 
                	'<span style="display: inline-block; overflow: hidden;white-space:nowrap;" >' + item.receiptPan + '</span></td><td>' +
                	'<span style="width: 160px; display: inline-block; overflow: hidden">' + item.settleNo + '</span></td><td>' +
                	'<span style="display: inline-block; overflow: hidden;white-space:nowrap;" >' + item.receiptName + '</span></td><td>' +
                	...
                	item.timeEnd + '</td><td>' +
                	item.dfStatus + '</td><td>' +
                	'<span class="span_auto_width">' + item.dfDesc + '</span></td><td>';
                	if(item.status != '00') {
                		html += '<a href="javascript:void(0);" onclick="reQueryAccountDetail(\'' + item.accountOrderNo + '\',\'' + item.mchtNo + '\')"><span class="text_blue span_auto_width">状态同步</span></a>&nbsp;&nbsp';
                	}
                	html += '</td></tr>';
	...
	//同步状态
	function reQueryAccountDetail(accountOrderNo, mchtNo) {
		...
		setTimeout(function(){
			$.ajax({
	            ...
	            type: "POST",
	            timeout: 10000,
	            url:"manage/reQueryAccountDetail?r=" + new Date().getTime(),
	            data:{"accountOrderNo": accountOrderNo, "mchtNo": mchtNo},// 你的formid
	            dataType : "text",
	            async: false,
	            error: function(request) {
					$("#loading", window.parent.document).hide();
